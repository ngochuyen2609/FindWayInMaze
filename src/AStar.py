...
class AStarWidget(QtWidgets.QWidget):
    ...
    def print_cell(self, x, y, color):
        ...
        print(f"Tọa độ tô màu: x1={x1}, y1={y1}, x2={x2}, y2={y2}, màu={color}")
    ...
    def tracePath(self, Path, color, onComplete=None):
        ...
        if len(Path) > 0:
            QTimer.singleShot(200, lambda: self.tracePath(Path, color, onComplete))
        else:
            if onComplete:
                QTimer.singleShot(200, onComplete)
    ...
    def solve(self, source, destination, typ):
        ...
        search_path, forward_path = MSA.aStar(self.maze_map, self.width, self.height, source, destination, typ)
        ...
        # Cập nhật thông tin
        self.path_len_label.setText(f"A Star Path Length: {len(forward_path) + 1}")
        self.search_len_label.setText(f"A Star Search Length: {len(search_path)}")
        self.scene.update() 
